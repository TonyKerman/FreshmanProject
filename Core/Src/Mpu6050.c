...
uint8_t Mpu6050_writeReg(Mpu6050 * me, uint16_t reg_addr, uint8_t data);
...
uint8_t MPU6050_readLen(Mpu6050 * me,uint8_t reg,uint8_t  len,uint8_t *pbuf);
...
uint8_t Mpu6050_Init(Mpu6050 * me) {
...
uint8_t MPU_Set_LPF(Mpu6050 * me,uint16_t lpf)
{
    ...
    return Mpu6050_writeReg(me,MPU_CFG_REG,data);//设置数字低通滤波器
}
...
//获取温度 单位 摄氏度
uint8_t Mpu6050_getTemp(Mpu6050 * me)
{
    uint8_t buf[2];
    uint8_t result = MPU6050_readLen(me,MPU_TEMP_OUTH_REG,2,buf);
    if(result == HAL_OK)
    {
...
//得到陀螺仪角速度值 单位 rad/秒
uint8_t Mpu6050_getGyroscope(Mpu6050 * me)
{
    uint8_t buf[6];
    int16_t rawData[3];
    //比率
    const double k[4]={0.00133125,0.0026625,0.005325,0.001065};

    uint8_t result =  MPU6050_readLen(me,MPU_GYRO_XOUTH_REG,6,buf);
    if(result == HAL_OK)
    {
        //16位ADC，原始值
        ...
        //单位转换
        me->gx =(double)rawData[0]*k[me->gFsr];
        me->gy = (double)rawData[1]*k[me->gFsr];
        me->gz = (double)rawData[2]*k[me->gFsr];
    }
...
//得到加速度值(带符号) 单位 米方/秒
uint8_t Mpu6050_getAccelerometer(Mpu6050 * me)
{
    uint8_t buf[6];
    int16_t rawData[3];
    //比率= (9.8*量程绝对值)/32767
    const double k[4]={0.000598163,0.001196326,0.002392652,0.004785304};

    uint8_t result = MPU6050_readLen(me,MPU_ACCEL_XOUTH_REG,6,buf);
    if(result == HAL_OK)
    {
...
uint8_t Mpu6050_writeReg(Mpu6050 * me, uint16_t reg_addr, uint8_t data)
{
    return HAL_I2C_Mem_Write(me->i2cHandle, me->writeAddr, reg_addr, I2C_MEMADD_SIZE_8BIT, &data, 1, 100);
}
...
uint8_t MPU6050_readLen(Mpu6050 * me,uint8_t reg,uint8_t len,uint8_t * pbuf)
{
    return HAL_I2C_Mem_Read(me->i2cHandle, me->readAddr, reg, I2C_MEMADD_SIZE_8BIT, pbuf, len, 0xfff);
}
...
int Mpup6050_update(Mpu6050 *me)
{
    ...
    const float Kp =100.0f; //比例增益支配率收敛到加速度计/磁强计
    const float Ki =0.005f; //积分增益支配率的陀螺仪偏见的衔接
    double halfT = me->dt/2;
    // 读取加速度计和陀螺仪数据
    Mpu6050_getAccelerometer(me);
    Mpu6050_getGyroscope(me);
